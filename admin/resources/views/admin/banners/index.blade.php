                            ...
                            @foreach($data as $row)
                                <tr id="row-{{ $row->id }}">
                                    ...
                                    <td>
                                        @can($module.'_edit')
                                        <label class="switch">
                                            {{ html()->checkbox('status', $row->status, null)->class('status')->id('status_'.$row->id)->attributes(['aria-label'=>'Toggle banner status', 'data-id'=>$row->id,'data-url'=>route('admin.'.$module.'.update.status',$row)]) }}
                                    ...
                                    <td>
                                        @can($module.'_edit')
                                            <a href="{{ route('admin.'.$module.'.edit',$row) }}" title="Edit banner" aria-label="Edit banner"><i class="fas fa-edit p-1" aria-hidden="true"></i></a>
                                        @endcan
                                        @can($module.'_delete')
                                            <a href="javascript:;" class="delete" title="Delete banner" aria-label="Delete banner" data-id="{{ $row->id }}" data-url="{{ route('admin.'.$module.'.destroy',$row) }}"><i class="fas fa-trash text-danger p-1" aria-hidden="true"></i></a>
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach
