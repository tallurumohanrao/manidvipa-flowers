            			...
            			<table class="table table-bordered  table-hover tablegrid">
                            ...
                            <?php $__currentLoopData = $data; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr id="row-<?php echo e($row->id); ?>">
                                    ...
                                    <td><?php echo e($row->usage_limit); ?></td>
                                    <td><?php echo e($row->usage_limit_per_user); ?></td>
                                    <td><?php echo e($row->minimum_purchage_amount); ?></td>
                                    <td><?php echo e($row->is_percentage_discount == 1 ? 'Yes' : 'No'); ?></td>
                                    <td><?php echo e($row->discount); ?></td>
                                    <td><?php echo e($row->start_date); ?></td>
                                    <td><?php echo e($row->end_date); ?></td>
                                    <td>
                                        <label class="switch">
                                        <?php echo e(html()->checkbox('status', $row->status, null)->class('status')->id('status_'.$row->id)->attributes(['data-id'=>$row->id,'data-url'=>route('admin.'.$module.'.update.status',$row->id)])); ?>
                                    ...
                                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check($module.'_edit')): ?>
                                            <a href="<?php echo e(route('admin.'.$module.'.edit',['coupon'=>$row->id])); ?>"><i class="fas fa-edit p-1"></i></a>
                                        <?php endif; ?>
                                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check($module.'_delete')): ?>
                                            <a href="javascript:;" class="delete" data-id="<?php echo e($row->id); ?>" data-url="<?php echo e(route('admin.'.$module.'.destroy',['coupon'=>$row->id])); ?>"><i class="fas fa-trash text-danger p-1"></i></a>
                                        <?php endif; ?>
                                    </td>
                                </tr>
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Projects\manidvipa-flowers\admin\resources\views/admin/coupons/index.blade.php ENDPATH**/ ?>