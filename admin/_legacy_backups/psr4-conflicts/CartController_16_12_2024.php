...
class CartController extends BaseController
{
    public function getcart(Request $request){
        ...
        foreach($carts as $value) :
            ...
            $products[] = ['cart_id'=>$value->id,'user_id'=>$value->user_id,'product_id'=>$product->id,'weight'=>$weight->name,'product_title'=>$product->title,'image'=>$image,'sell_price'=>$weight->sell_price,'list_price'=> $weight->list_price,'quantity'=>$value->quantity];
        ...
        if($coupon_row){
            ...
            $totals['coupon'] = ['title' => "Coupon (".$coupon_row->title.")", 'amount' => $coupon_discount];
        }

        if($vat){
            $totals['gst'] = ['title' => 'GST', 'amount' => ($subTotal * $vat)/100];
        }
        ...
        return response()->json(['success' => true,'data'=> $products,'totals'=> $totals], 200);
    }
            ...
            if($cart = DB::table('carts')->where('id',$product['cart_id'])->where(function($cwq) use($cart_session,$user_id){
                ...
                if($cart->product_id){
                    ...
                    if($weight->stock && ($weight->qty < $request->quantity)){
                        $errors[$cart->id][] = $product['product_title'].' with weight'.$weight->name.' Stock not available. Available stock is ('.$weight->qty .').';
                    }else{
    ...
    public function addCoupon(Request $request)
    {
        ...
        if($coupon->minimum_purchage_amount){
            ...
            if($subTotal < $coupon->minimum_purchage_amount ){
                ...
                return response()->json(['success'=>false,'message'=>"Order amount should be minimum $amount to avail coupon."]);
            }
        }
        //start date
    ...
    public function getCartCoupon(Request $request){
        ...
        $coupon = DB::table('cart_line_items as cli')->select('cli.id as cart_coupon_id','c.title','c.coupon_code','c.discount','c.is_percentage_discount')->where('cli.cart_session',$request->cart_session)->join('coupons as c','c.id','=','cli.value_id')->orderByDesc('cli.id')->first();
